...
export const metadata = { title: "Cross-channel shortage — Kaviari Cellar" };
...
export default async function ShortagePage() {
  ...
  const cases = await prisma.scmShortageCase.findMany({
    include: {
      product: true,
                    ...
                    <TableRow key={entry.id}>
                      ...
                      <TableCell>
                        <Button size="sm" variant="outline" asChild>
                          <Link href={`/scm/sales/shortage/${entry.id}`}>
                            {["open", "pending_approval"].includes(entry.status)
                              ? "Decide"
                              : "Open"}
                          </Link>

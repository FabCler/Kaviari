...
import { InvoiceVerify } from "@/components/scm/purchasing/invoice-verify";
...
export async function generateMetadata({
  ...
  return {
    title: `${invoice?.invoiceNumber ?? "Invoice"} — Kaviari Cellar`,
  };
}
        ...
        actions={
          <>
            <Button variant="outline" asChild>
              <Link href="/scm/purchasing/invoices">All invoices</Link>
            </Button>
            {invoice.poId ? (
              <Button variant="outline" asChild>
                <Link href={`/scm/trace/po/${invoice.poId}`}>Open the PO</Link>
              </Button>
            ) : null}
          </>
        }
      ...
      <InvoiceVerify
        ...
        poLines={
          invoice.po?.lines.map((line) => ({
            id: line.id,
            label: `${line.product.prCode} · ${line.baseQuantity} ${line.product.unit} @ ${line.unitPrice}`,
          })) ?? []
        }
      />

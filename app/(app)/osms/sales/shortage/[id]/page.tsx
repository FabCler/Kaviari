...
import { proposeShortageSplit } from "@/lib/osms/shortage";
...
import { ShortageDecision } from "@/components/osms/sales/shortage-decision";
...
export async function generateMetadata({
  ...
  const entry = await osms.shortageCase.findUnique({
    where: { id },
    select: { caseNumber: true },
  });
  return { title: `${entry?.caseNumber ?? "Shortage"}` };
}
        ...
        actions={
          <>
            <Button variant="outline" asChild>
              <Link href="/osms/sales/shortage">All cases</Link>
            </Button>
            {entry.poLine ? (
              <Button variant="outline" asChild>
                <Link href={`/osms/trace/po/${entry.poLine.poId}`}>
                  Document trace
                </Link>
              </Button>
            ) : null}

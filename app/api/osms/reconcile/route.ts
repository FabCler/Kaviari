...
import { runSoReconciliation, syncPoStatuses } from "@/lib/osms/workflow";
import {
...
async function handlePoInvoice(body: z.infer<typeof poInvoiceSchema>) {
  ...
  const recon = await osms.poInvoiceRecon.findUnique({
    where: { id: body.id },
...
async function handleSalesReview(body: z.infer<typeof salesSchema>) {
  ...
  const recon = await osms.soPoRecon.findUnique({
    where: { id: body.id },
  ...
  if (stillOpen === 0 && recon.poLine) {
    await notify({
      ...
      link: `/osms/sales/allocation?po=${recon.poLine.poId}`,
    });
  }

...
import { nextAllocationNumber } from "@/lib/osms/numbering";
import { validateAllocation, type AllocationLineInput } from "@/lib/osms/allocation";
...
import { syncPoStatuses } from "@/lib/osms/workflow";
...
export async function POST(request: Request) {
  ...
  const poLine = await osms.purchaseOrderLine.findUnique({
  ...
  await osms.allocationLine.createMany({
    data: lines.map((line) => ({
  ...
  if (validation.totals.warehouseQuantity > 0) {
    await raiseException({
      ...
      documentNumber: allocationNumber,
      productId: poLine.productId,
      description: `${validation.totals.warehouseQuantity} ${poLine.product.unit} of ${poLine.product.code} going to stock instead of a customer.`,
  ...
  if (body.complete) {
    ...
    await notify({
      ...
      documentId: allocation.id,
      documentNumber: allocationNumber,
      link: `/osms/warehouse/receiving?po=${poLine.poId}`,
    });
  }

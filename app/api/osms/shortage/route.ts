...
import { validateShortageDecision } from "@/lib/osms/shortage";
import { compareSoConfirmed } from "@/lib/osms/reconcile";
...
export async function POST(request: Request) {
  ...
  const shortageCase = await osms.shortageCase.findUnique({
  ...
  if (body.action === "reject") {
    ...
    await osms.shortageCase.update({
  ...
  for (const entry of merged) {
    ...
    await osms.shortageAllocation.update({
    ...
    if (existing) {
      await osms.soPoRecon.update({ where: { id: existing.id }, data: payload });
    } else {
    ...
    await notify([
      {
        ...
        link: `/osms/sales/allocation?po=${shortageCase.poLine.poId}`,
      },
    ]);
  }

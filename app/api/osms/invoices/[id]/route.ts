...
import { matchInvoiceToPo } from "@/lib/osms/import/invoice";
import { runPoInvoiceReconciliation, syncPoStatuses } from "@/lib/osms/workflow";
...
export async function PATCH(
  ...
  const products = productIds.length
    ? await osms.product.findMany({ where: { id: { in: productIds } } })
    : [];
  ...
  for (const input of body.lines ?? []) {
    ...
    await osms.invoiceLine.update({ where: { id: line.id }, data });
    await recordAudit(
  ...
  if (body.action === "reject") {
    ...
    const refreshed = await osms.invoice.findUnique({
      where: { id },
      include: { lines: true },
    });
  ...
  if (Object.keys(headerData).length > 0) {
    await osms.invoice.update({ where: { id }, data: headerData });
  }

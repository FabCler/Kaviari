...
export async function POST(request: Request) {
  ...
  const exception = await prisma.scmException.create({
    data: {
      code,
      type: body.type,
...
export async function PATCH(request: Request) {
  ...
  const existing = await prisma.scmException.findUnique({ where: { id } });
  if (!existing) {
  ...
  await prisma.scmException.update({ where: { id }, data });
  await recordAudit(
    {
      entity: "exception",
      entityId: id,

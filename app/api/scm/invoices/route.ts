...
import { extractInvoice, matchInvoiceToPo } from "@/lib/scm/import/invoice";
...
export async function POST(request: Request) {
  ...
  let po = poIdInput
    ? await prisma.scmPurchaseOrder.findUnique({ where: { id: poIdInput } })
    : null;
  if (!po && extraction.poNumber) {
    po = await prisma.scmPurchaseOrder.findUnique({
  ...
  const supplier =
    (po
      ? await prisma.supplier.findUnique({ where: { id: po.supplierId } })
      : null) ??
  ...
  const duplicate = supplier
    ? await prisma.scmInvoice.findFirst({
  ...
  const invoice = await prisma.scmInvoice.create({
    data: {
      invoiceNumber,
  ...
  await prisma.scmAttachment.create({
    data: {
      entity: "invoice",
      entityId: invoice.id,
      fileName: file.name,
  ...
  await notify({
    ...
    documentId: invoice.id,
    documentNumber: invoiceNumber,
    link: `/scm/purchasing/invoices/${invoice.id}`,
  });

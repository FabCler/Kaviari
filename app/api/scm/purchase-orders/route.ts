...
import { nextPoNumber } from "@/lib/scm/numbering";
...
import { ORDER_ADJUSTMENT_REASONS } from "@/lib/scm/domain";
...
export async function POST(request: Request) {
  ...
  const [prLines, soLines] = await Promise.all([
    prLineIds.length
      ? prisma.scmPurchaseRequestLine.findMany({ where: { id: { in: prLineIds } } })
      : Promise.resolve([]),
    soLineIds.length
      ? prisma.scmSalesOrderLine.findMany({ where: { id: { in: soLineIds } } })
      : Promise.resolve([]),
  ]);
  ...
  const existing = await prisma.scmPurchaseOrder.findUnique({
    where: { poNumber },
  });
  if (existing) {
    return Response.json(
...
export async function PATCH(request: Request) {
  ...
  const po = await prisma.scmPurchaseOrder.findUnique({ where: { id } });
  ...
  await prisma.scmPurchaseOrder.update({ where: { id }, data });
  await auditEvent(

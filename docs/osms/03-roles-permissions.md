...
`●` = ทำได้ · `–` = ทำไม่ได้ (ที่มา: `lib/osms/permissions.ts`)
...
| Page | `app/(app)/osms/**/page.tsx` | เรียก `can(actor, …)` — ไม่ผ่านแสดง `<NoAccess />` และซ่อนปุ่มลงมือ |
| Query | `currentScope()` + `narrowScope()` | กรอง channel ในทุก query ที่แตะ demand ของลูกค้า |
| API | `app/api/osms/**/route.ts` | `requirePermission()` → 401/403 ก่อนแตะฐานข้อมูล |

**การซ่อนปุ่มไม่ใช่การควบคุมสิทธิ์** — ทุก route handler ตรวจซ้ำเสมอ
ต่อให้ client ยิงตรงเข้ามาก็ถูกปฏิเสธ

```ts
// app/api/osms/purchase-orders/route.ts

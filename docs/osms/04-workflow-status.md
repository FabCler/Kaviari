...
`lib/osms/status.ts`) แล้วเก็บลงคอลัมน์เพื่อให้ filter/index ได้ — เอกสารคือ
...
ครอบคลุมด้วยเทสต์ใน `tests/osms-status.test.ts`
...
หน้ารายละเอียดเอกสาร (`/osms/trace/…`) แสดงขั้นตอนตาม §19:

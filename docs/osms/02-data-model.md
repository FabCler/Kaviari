...
`lib/osms/trace.ts` เดินเส้นทางนี้จาก PO, SO หรือ PR ก็ได้ แล้วคืนทุก node
พร้อมสถานะ — หน้า `/osms/trace/[type]/[id]` แสดงผลตรงนี้
...
#### `departments` / `roles` (Department / Role)
...
Field พื้นฐานอื่น: `code` (รหัสสินค้า — unique, จับคู่กับ `ItemCode` ของ ERP),
`name`, `nameTh`, `category` (ประเภทสินค้า — String ไม่ใช่ enum เพิ่มกลุ่มใหม่
ได้โดยไม่ต้อง migrate), `unit` (หน่วยคลัง — เป็นหน่วยกลางของทุกการเปรียบเทียบ),
`gramsPerUnit`, `active`

#### `users` (User)

ตารางของ OSMS เอง **ไม่มีรหัสผ่าน** — การ authenticate เป็นหน้าที่ของแอปที่
host และ `lib/osms/access.ts` จับคู่ด้วยอีเมล

| Field | Type | คำอธิบาย |
|---|---|---|
| `email` | String | unique — คีย์เดียวที่ใช้จับคู่กับระบบ login ภายนอก |
| `name` | String | ชื่อที่แสดงในเอกสารและ audit |
...
| `active` | Boolean | ปิดการใช้งานได้โดยไม่ลบประวัติ |

### 2.2 Demand

#### `purchase_requests` (PurchaseRequest) / `purchase_request_lines`
...
#### `sales_orders` (SalesOrder) / `sales_order_lines`
...
#### `po_invoice_reconciliation` (PoInvoiceRecon)
...
#### `so_po_reconciliation` (SoPoRecon)
...
#### `allocation` (Allocation) / `allocation_lines`
...
#### `receiving` (Receiving) / `receiving_lines` / `receiving_items`
...
#### `shipment` (Shipment) / `shipment_lines`
...
#### `warehouse_stock_transactions` (WarehouseStockTransaction)
...
#### `shortage_allocations` (ShortageAllocation)
...
**ฐานข้อมูลแยกทั้งก้อน** — OSMS มี datasource ของตัวเอง (`OSMS_DATABASE_URL`)
และ client ของตัวเอง (`lib/generated/osms`) จึงไม่มี prefix กัน collision อีก
ต่อไป: model ชื่อ `PurchaseOrder`, `SalesOrder`, `Invoice` ตรง ๆ และ `@@map`
ให้ชื่อตารางจริงตรงตามที่ §20 กำหนด (`purchase_orders`, `invoices`, …)
ตาราง `products` / `users` / `settings` ก็เป็นของ OSMS เอง ไม่ได้ยืมของใคร

**ผู้ใช้มาจากไหน** — `users` ของ OSMS เก็บเฉพาะสิ่งที่ OSMS ต้องใช้ (email,
name, department, allChannels) ไม่มีรหัสผ่าน การ authenticate เป็นหน้าที่ของ
แอปที่ host แล้ว `lib/osms/access.ts` จับคู่บัญชีนั้นเข้ากับ operator ของ OSMS
ด้วย **อีเมล** บัญชีใหม่จะได้ department `none` ซึ่ง permission matrix อ่านว่า
"ยังไม่มีสิทธิ์" — ต้องมีคนกำหนด department ให้เสมอ ไม่ได้สิทธิ์เพราะ login

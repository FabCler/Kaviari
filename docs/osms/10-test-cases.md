...
| ไฟล์ | ครอบคลุม |
|---|---|
| `tests/osms-reconcile.test.ts` | variance, qty/price status, tolerance, `confirmedQuantity()` (§14), Invoice/SO, PO vs SO ทั้ง 5 สถานะ |
| `tests/osms-allocation.test.ts` | สมการการจัดสรร, การบังคับ location/reason/dept, over-allocation, การชั่งรายชิ้น |
| `tests/osms-gate.test.ts` | ด่านทั้ง 6 แต่ละด่านทั้งกรณีผ่านและไม่ผ่าน + ลำดับการรายงาน |
| `tests/osms-units.test.ts` | การแปลงหน่วยสองทาง, กฎเฉพาะสินค้าชนะกฎกลาง, การปฏิเสธเมื่อไม่มี conversion |
| `tests/osms-status.test.ts` | 22 สถานะ, `resolveStatus()`, partial vs fully received, EXCEPTION/REJECTED, กติกาการเปลี่ยนสถานะ |
| `tests/osms-channels.test.ts` | channel scoping, `narrowScope()` (กัน URL ขยายสิทธิ์), permission matrix รวม Sales Manager |
| `tests/osms-shortage.test.ts` | ข้อเสนอการแบ่งข้าม channel ตาม §45, การตรวจการอนุมัติ |
| `tests/osms-tolerance-sla.test.ts` | ลำดับ tolerance (supplier → channel → product type → global), SLA on track/due soon/overdue |
...
| 12 | ใครก็ได้ | เปิด `/osms/trace/po/<id>` | stepper เขียวทั้งแถว, ตารางจำนวนครบทุกคอลัมน์, audit trail ครบ |
...
|---|---|---|---|
| 1 | ผู้ใช้ใหม่ (department = none) | เปิด `/osms` | หน้า "No access yet" พร้อมวิธีขอสิทธิ์ |
...
| 4 | Warehouse | ยิง `POST /api/osms/purchase-orders` ตรง ๆ | **403** `Your department (warehouse) is not allowed to purchasing → createPo` |
| 5 | Sales | ยิง `POST /api/osms/receiving` ตรง ๆ | **403** |
| 6 | Management | เปิดทุกกระดาน | เห็นข้อมูลครบ แต่ไม่มีปุ่มลงมือ |
| 7 | Management | เปิด `/osms/master-data` | "No access yet" (admin เท่านั้น) |
...
| 4 | เปิด `/osms/audit` ค้น `PO-2026-0002` | เห็นทุกแถวเรียงใหม่ไปเก่า |
| 5 | กด Export Excel | ไฟล์มีคอลัมน์ When / User / Department / Document / Entity / Action / Field / Old / New / Reason |
| 6 | เปิด `/osms/trace/po/<id>` | ส่วน "Audit trail for PO-…" แสดงเฉพาะของเอกสารใบนั้น |
...
| 2 | Sales (FS) | ยิง `POST /api/osms/allocations` ตรง ๆ | **409** `Cross-channel shortage SHT-2026-0001 is waiting for a management decision` |
| 3 | Sales (FS) | ยิง `POST /api/osms/shortage` | **403** — Sales ของ channel เดียวอนุมัติไม่ได้ |
| 4 | Management | เปิด `/osms/sales/shortage/<id>` | เห็น demand แยกตาม channel, ช่อง Approved **ว่างเปล่า**, ปุ่ม Approve ถูกปิด |

...
`lib/osms/exceptions.ts → raiseException()` เป็นทางเดียวที่สร้าง exception และ
...
`lib/osms/notify.ts` เขียนแจ้งเตือนไปยังแผนกที่ต้องลงมือ:
...
แจ้งเตือนที่ยังไม่อ่านแสดงบน `/osms` แยกตามแผนกของผู้ใช้ (admin/management
เห็นทุกแผนก)

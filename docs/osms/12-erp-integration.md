...
| **`@@map` ชื่อตารางมาตรฐาน** | ตารางจริงชื่อ `purchase_orders`, `invoices`, … ต่อ view/ETL ได้ตรงไปตรงมา |
| **ฐานข้อมูลแยกของตัวเอง** | ย้าย OSMS ไปอยู่ข้าง ERP ได้โดยไม่ลากตารางของแอปอื่นไปด้วย — เปลี่ยนแค่ `OSMS_DATABASE_URL` |
| **`lib/osms/access.ts` เป็นจุดเชื่อมเดียวกับ identity ภายนอก** | เปลี่ยน resolver ตัวเดียวก็ต่อกับ SSO / HR ของ ERP ได้ ไม่ต้องแตะที่อื่น |
...
flowchart LR
  subgraph scm["โมดูลนี้"]
    IMP["lib/osms/import/**<br/>validate + commit"]
    DOM["lib/osms/**<br/>business rules"]
    DB[("Database")]
  end
...
`lib/osms/import/validate.ts` รับอยู่แล้ว แล้วเรียก `commit*Import()` ตัวเดิม —
...
| Adapter | `lib/osms/erp/sap-b1.ts` — pull master + open PO/SO, push GRPO |
...
1. เปลี่ยน `provider = "postgresql"` ใน `prisma/osms/schema.prisma`
2. ตั้ง `OSMS_DATABASE_URL`
3. `npx prisma migrate deploy --schema prisma/osms/schema.prisma`

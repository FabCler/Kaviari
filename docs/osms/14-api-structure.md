...
ทุก endpoint อยู่ใต้ `/api/osms/**` เป็น Next.js Route Handler
...
### `POST /api/osms/invoices` — อัปโหลด Invoice (§5.3)
...
### `PATCH /api/osms/invoices/[id]` — ตรวจ/แก้/ยืนยัน
...
### `POST /api/osms/purchase-orders` — เปิด PO
...
### `PATCH /api/osms/purchase-orders` — เปลี่ยนสถานะ/หมายเหตุ
Permission `purchasing.editPo` · `{ id, status?, notes?, reason? }`

### `POST /api/osms/reconcile` — จุดตัดสินใจของคน
...
### `POST /api/osms/warehouse-stock` — ย้าย stock (§24)
...
### `DELETE /api/osms/master/[entity]?id=…`
ปิดการใช้งาน (`active=false`) ไม่ลบจริง ยกเว้น `conversions` ที่ลบได้

### `POST` / `PATCH /api/osms/exceptions` (§26)

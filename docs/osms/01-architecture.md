...
OSMS เป็น **ระบบแยกของตัวเอง** ที่บังเอิญ deploy อยู่ใน Next.js app เดียวกับ
แอปอื่น — แต่ไม่ได้ใช้ตารางร่วมกัน:

| สิ่งที่แยก | ที่อยู่ |
|---|---|
| Schema | `prisma/osms/schema.prisma` (datasource ของตัวเอง) |
| Connection string | `OSMS_DATABASE_URL` |
| Prisma client | `lib/generated/osms` เรียกผ่าน `lib/osms/db.ts` (`osms`) |
| Master data | `products`, `users`, `settings` เป็นของ OSMS เอง |
| Seed | `prisma/osms/seed.ts` — `npm run osms:setup` |
| ธีม | `.osms-theme` ใน `app/globals.css` (slate #384B59 × blush #E1A59B) |

จุดเชื่อมกับแอปที่ host มีจุดเดียว: **อีเมลของบัญชีที่ login** ซึ่ง
`lib/osms/access.ts` ใช้หา operator ของ OSMS แล้วอ่าน department/channel
จากตาราง `users` ของ OSMS เอง — จุดเดียวกันนี้คือจุดที่จะถูกแทนที่ตอนต่อ
SAP Business One (ดู [12-erp-integration.md](12-erp-integration.md))
...
flowchart TB
  ...
  subgraph server["Next.js server (App Router)"]
    PAGES["app/(app)/osms/** — pages<br/>read through lib/osms/queries.ts"]
    API["app/api/osms/** — route handlers<br/>zod validation + permission guard"]
    DOMAIN["lib/osms/** — domain layer<br/>channels · status · permissions · units<br/>reconcile · shortage · allocation · gate<br/>tolerance · sla · warehouse-stock · reports<br/>workflow · audit"]
  end
...
| Layer | หน้าที่ | ไฟล์ |
|---|---|---|
| **Presentation** | หน้าจอ 16 หน้า, sidebar แยกตามแผนก | `app/(app)/osms/**`, `components/osms/**` |
| **API** | รับคำสั่งเปลี่ยนสถานะ ตรวจสิทธิ์และ validate ด้วย zod | `app/api/osms/**` |
| **Domain** | กฎธุรกิจทั้งหมด เป็นฟังก์ชันบริสุทธิ์ที่ทดสอบได้ | `lib/osms/*.ts` |
| **Persistence** | Prisma models 33 ตาราง | `prisma/schema.prisma` |
| **Integration** | อ่านไฟล์ Excel/CSV/PDF, เรียก AI สำหรับ Invoice | `lib/osms/import/**`, `lib/import/parse.ts` |
...
`app/api/osms/receiving/route.ts` เรียกซ้ำก่อนบันทึกเสมอ แม้หน้าจอจะซ่อนปุ่มไว้แล้ว

### Security

- Session: HMAC-signed cookie (ของเดิม) → `getCurrentUser()`
- Authorization: `lib/osms/permissions.ts` — matrix ตามแผนก
  ทุก route handler เรียก `requirePermission()` ก่อนทำงาน
- Audit: `lib/osms/audit.ts` เขียนทุกการเปลี่ยนแปลงพร้อม user/reason
...
sequenceDiagram
  ...
  participant API as /api/osms/*
  participant W as lib/osms/workflow.ts

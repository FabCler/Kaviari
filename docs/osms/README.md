...
ระบบบริหารการสั่งซื้อ – รับสินค้า – ตรวจสอบ Invoice – จัดสรรสินค้า – ส่งสินค้าให้ลูกค้า

เอกสารออกแบบทั้งหมด พร้อมแผนที่ว่าแต่ละ deliverable ถูกทำจริงที่ไฟล์ไหน

> **OSMS เป็นระบบแยกของตัวเอง** — มี database ของตัวเอง
> (`prisma/osms/schema.prisma` + `OSMS_DATABASE_URL`), Prisma client ของตัวเอง
> (`lib/generated/osms`) และ master data ของตัวเอง (Product, User, Setting)
> ไม่ได้ใช้ตารางร่วมกับแอปที่ deploy อยู่ข้างกัน จุดเชื่อมมีจุดเดียวคือ
> **อีเมลของผู้ใช้ที่ login** (`lib/osms/access.ts`) ซึ่งเป็นจุดเดียวกับที่จะ
> ถูกแทนที่เมื่อไปต่อกับ SAP Business One หรือ ERP อื่น
...
app/api/osms/       route handlers (import, invoices, purchase-orders,
                   reconcile, allocations, receiving, shipments, master,
                   exceptions, notifications, settings, exports)
app/(app)/osms/     the screens
components/osms/    the module's UI components
prisma/osms/seed.ts sample data (four scenarios)
tests/osms-*.test.ts unit tests for the rules above
```
...
1. **จำนวนที่ยืนยันล่าสุดคือความจริง (§14).** ทุกขั้นตอนถัดไปใช้
   ...
   `confirmedQuantity()` ใน `lib/osms/reconcile.ts` เป็นที่เดียวที่ตัดสินใจเรื่องนี้
2. **ห้ามข้ามขั้นตอน (§21).** ประตู 6 ด่าน (`lib/osms/gate.ts`) เป็นฟังก์ชัน

...
โค้ดทุกจุดที่กรองตาม channel ใช้ `lib/scm/channels.ts` ซึ่งอ่านจากตาราง
...
2. ถ้าผู้ใช้ใส่ `?channel=<id>` ที่ตัวเองไม่มีสิทธิ์ → `narrowScope` ยุบเหลือ scope ว่าง
   **ไม่ใช่กลับไปเป็นมุมมองที่ไม่กรอง** (มีเทสต์ครอบเคสนี้ใน `tests/scm-channels.test.ts`)
...
`lib/scm/shortage.ts → detectCrossChannelShortage()` สร้าง case เมื่อ **ทั้งสองข้อ**
เป็นจริง:

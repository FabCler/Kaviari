...
import { parseImportDate, parseNumber, type RowSet } from "@/lib/scm/import/rows";
import type { DEMAND_COLUMNS, PO_COLUMNS, SO_COLUMNS } from "@/lib/scm/import/columns";
...
export async function validateDemandRows(
  ...
  const [existingPrs, existingSos, existingPos] = await Promise.all([
    prisma.scmPurchaseRequest.findMany({ select: { prNumber: true } }),
    prisma.scmSalesOrder.findMany({ select: { soNumber: true } }),
    prisma.scmPurchaseOrder.findMany({ select: { poNumber: true } }),
  ]);
...
export async function validatePoRows(
  ...
  const [products, suppliers] = await Promise.all([
    prisma.product.findMany({ where: { prCode: { in: codes } } }),

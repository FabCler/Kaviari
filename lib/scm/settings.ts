...
export const SCM_SETTING_KEYS = {
  qtyTolerancePct: "scmQtyTolerancePct",
  priceTolerancePct: "scmPriceTolerancePct",
  deliveryWarningDays: "scmDeliveryWarningDays",
  defaultStorageLocation: "scmDefaultStorageLocation",
} as const;

export const SCM_DEFAULTS: ScmSettings = {
...
export async function getScmSettings(): Promise<ScmSettings> {
...
export async function saveScmSettings(
  patch: Partial<ScmSettings>
): Promise<void> {
  const entries = Object.entries(patch).filter(([, value]) => value != null);
  for (const [key, value] of entries) {
    const settingKey = SCM_SETTING_KEYS[key as keyof ScmSettings];

  ...
  for (const line of po.lines) {
    for (const link of line.demandLinks) {
      ...
      if (link.prLine) {
        push({
          ...
          href: `/osms/trace/pr/${link.prLine.pr.id}`,
        });
      }
    }
    ...
    for (const receivingLine of line.receivingLines) {
      push({
        ...
        href: `/osms/warehouse/receiving/${receivingLine.receiving.id}`,
      });
    }
    ...
    for (const allocation of line.allocations) {
      push({
        ...
        href: `/osms/sales/allocation/${allocation.id}`,
      });
      for (const allocationLine of allocation.lines) {
        ...
        for (const shipmentLine of allocationLine.shipmentLines) {
          push({
            ...
            href: `/osms/warehouse/shipments/${shipmentLine.shipment.id}`,
          });
        }
      }
    }
...
export async function traceSo(soId: string): Promise<TraceResult | null> {
  const so = await osms.salesOrder.findUnique({
    where: { id: soId },
    include: {
      customer: true,
  ...
  const poLine = poLineIds.length
    ? await osms.purchaseOrderLine.findFirst({
  ...
  nodes.so.push(root);
  const full = await osms.salesOrder.findUnique({
    where: { id: soId },
...
export async function tracePr(prId: string): Promise<TraceResult | null> {
  const pr = await osms.purchaseRequest.findUnique({
    where: { id: prId },
    include: {

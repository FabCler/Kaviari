...
async function currentNumbers(kind: string, prefix: string): Promise<string[]> {
  ...
  switch (kind) {
    case "PO": {
      const rows = await osms.purchaseOrder.findMany({
    ...
    case "RCV": {
      const rows = await osms.receiving.findMany({
    ...
    case "ALC": {
      const rows = await osms.allocation.findMany({
    ...
    case "SHP": {
      const rows = await osms.shipment.findMany({
    ...
    case "EXC": {
      const rows = await osms.exception.findMany({
        where: { code: where },
        select: { code: true },
      });
    ...
    case "PR": {
      const rows = await osms.purchaseRequest.findMany({
    ...
    default: {
      const rows = await osms.salesOrder.findMany({

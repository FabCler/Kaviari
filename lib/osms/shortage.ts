...
export async function detectCrossChannelShortage(
  ...
  const existing = await osms.shortageCase.findFirst({
    where: {
      poLineId,
  ...
  const created = await osms.shortageCase.create({
    data: {
      caseNumber,
...
async function nextShortageNumber(date = new Date()): Promise<string> {
  ...
  const rows = await osms.shortageCase.findMany({
...
export async function openShortageCases(poId: string) {
  return osms.shortageCase.findMany({
    where: {
      poLine: { poId },
      status: { in: ["open", "pending_approval"] },
    },

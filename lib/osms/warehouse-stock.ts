...
export async function receiveIntoStock(
  ...
  const stock = await client.warehouseStock.create({
    data: {
      stockNumber,
  ...
  await client.warehouseStockTransaction.create({
    data: {
      stockId: stock.id,
      type: "in",
      quantity,
...
export async function listWarehouseStock(filters: StockFilters = {}) {
  return osms.warehouseStock.findMany({
    where: {

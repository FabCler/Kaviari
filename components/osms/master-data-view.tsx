...
import type { Settings } from "@/lib/osms/settings";
...
async function post(entity: string, body: unknown): Promise<boolean> {
  const response = await fetch(`/api/osms/master/${entity}`, {
    method: "POST",
  ...
  return (
    <Card>
      <CardHeader>
        ...
        <p className="text-sm text-muted-foreground">
          The purchasing fields on the OSMS product master: Thai name,
                ...
                <TableRow key={row.id}>
                  ...
                  <TableCell>
                    <Input
                      ...
                      aria-label={`Thai name for ${row.code}`}
                    />
                  </TableCell>
                  ...
                  <TableCell>
                    <Input
                      ...
                      aria-label={`Purchase unit for ${row.code}`}
                    />
                  </TableCell>
                  <TableCell>
                    <Input
                      ...
                      aria-label={`Conversion for ${row.code}`}
                    />
                  </TableCell>
                  <TableCell>
                    <Input
                  ...
                  <TableCell>
                    ...
                    >
                      <SelectTrigger aria-label={`Default supplier for ${row.code}`}>
                        <SelectValue placeholder="-" />
                      </SelectTrigger>
                      <SelectContent>
                  ...
                  <TableCell>
                    <Switch
                      ...
                      aria-label={`${row.code} weighed per piece`}
...
function TolerancePanel({
  ...
  async function save() {
    setBusy(true);
    try {
      const response = await fetch("/api/osms/settings", {
        method: "PATCH",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({

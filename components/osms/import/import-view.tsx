...
function SpreadsheetImport({
  ...
  async function analyze() {
    ...
    try {
      ...
      form.set("kind", kind);
      form.set("file", file);
      const response = await fetch("/api/osms/import", {
        method: "POST",
        body: form,
      });
...
function InvoiceImport({
  ...
  async function upload() {
    ...
    try {
      ...
      const response = await fetch("/api/osms/invoices", {
        method: "POST",
        body: form,
      });
      const payload = await response.json();
      if (!response.ok) {
        ...
        if (payload.invoiceId) {
          router.push(`/osms/purchasing/invoices/${payload.invoiceId}`);
        }
        return;
      }
      ...
      router.push(`/osms/purchasing/invoices/${payload.id}`);
    } catch {
